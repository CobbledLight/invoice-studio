...
export default function Studio() {
    ...
    function demoAction(i: Invoice, a: string): Invoice { if (a === 'duplicate')
        ...
        if (i.status !== 'issued')
            throw Error('Only an issued invoice can be sent.');
        return { ...i, status: 'sent' };
    } if (a === 'pay') {
        if (i.status !== 'sent')
            throw Error('Only a sent invoice can be marked paid.');
        ...
        return { ...i, status: 'sent', paid_on: null }; if (a === 'cancel') {
        if (i.status !== 'issued' && i.status !== 'sent')
            throw Error('Only an issued or sent invoice can be cancelled.');
        if (!reason.trim())
    ...
    const sum = (s: string) => curInvoices.filter(i => i.status === s || s === 'issued' && i.status === 'sent').reduce((n, i) => n.plus(i.total), new Decimal(0)).toFixed(2);
