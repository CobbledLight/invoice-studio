...
export function AuthScreen({ onDemo, recovery = false, onRecovered }: {
    ...
    async function signInWithGoogle() { setError(''); setBusy(true); try {
        if (!supabase)
            throw Error('Account connection is not configured yet.');
        const { error: oauthError } = await supabase.auth.signInWithOAuth({ provider: 'google', options: { redirectTo: location.origin } });
        if (oauthError)
            throw oauthError;
    }
    catch (e) {
        const detail = e instanceof Error ? e.message : '';
        setError(detail.includes('Unsupported provider') ? 'Google login is not enabled in Supabase yet. Enable the Google provider in Supabase Authentication settings, then try again.' : detail || 'Unable to connect with Google. Please retry.');
        setBusy(false);
    } }
    return <main className="auth-wrap"><section className="auth-card"><Brand /><h1>{recovery ? 'Choose a new password' : mode === 'signup' ? 'Make room for your work.' : mode === 'reset' ? 'Reset your password' : 'Welcome back.'}</h1><p className="muted">{recovery ? 'Set a secure password to continue.' : mode === 'signup' ? 'Create your private invoice workspace.' : mode === 'reset' ? 'We’ll email you a reset link.' : 'Your invoices, customers and details. All together.'}</p>{!recovery && mode === 'login' && <Button type="button" variant="outline" className="google-button" onClick={signInWithGoogle} disabled={busy}>G <span>Continue with Google</span></Button>}<form onSubmit={submit}>{error && <div role="alert" className="error-message">{error}</div>}{message && <div role="status" className="success-note">{message}</div>}{!recovery && <Field label="Email address" value={email} onChange={setEmail} type="email" required/>}{(recovery || mode !== 'reset') && <Field label="Password" value={password} onChange={setPassword} type="password" required/>}{(mode === 'signup' || recovery) && <p className="inline-help">Use at least 8 characters.</p>}<Button type="submit" className="primary" disabled={busy || ((mode === 'signup' || recovery) && password.length < 8)}>{busy ? 'Please wait…' : recovery ? 'Save password' : mode === 'signup' ? 'Create account' : mode === 'reset' ? 'Send reset link' : 'Sign in'}<ArrowRight size={17}/></Button></form>{!recovery && <><div className="auth-links"><button className="text-button" onClick={() => switchMode(mode === 'login' ? 'signup' : 'login')}>{mode === 'login' ? 'Create an account' : 'Back to sign in'}</button>{mode === 'login' && <button className="text-button" onClick={() => switchMode('reset')}>Forgot password?</button>}</div><div style={{ borderTop: '1px solid #e4ebe7', marginTop: 28, paddingTop: 22, textAlign: 'center' }}><button className="text-button" onClick={onDemo}>Explore a sample workspace →</button><p className="inline-help">Sample changes reset when you leave or refresh.</p></div></>}</section></main>;
}
